...
/**
 * Table class for horas_trabajadas
 */
class HorasTrabajadas extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id;
    public DbField $socio_id;
    public DbField $fecha;
    public DbField $horas;
    public DbField $tarea;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "horas_trabajadas"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // socio_id
        $this->socio_id = new DbField(
            $this, // Table
            'x_socio_id', // Variable name
            'socio_id', // Name
            '`socio_id`', // Expression
            '`socio_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`socio_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->socio_id->InputTextType = "text";
        $this->socio_id->Raw = true;
        $this->socio_id->Nullable = false; // NOT NULL field
        $this->socio_id->Required = true; // Required field
        $this->socio_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->socio_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['socio_id'] = &$this->socio_id;
        ...
        $this->fecha->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $this->language->phrase("IncorrectDate"));
        ...
        $this->horas->DefaultErrorMessage = $this->language->phrase("IncorrectFloat");
        $this->horas->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['horas'] = &$this->horas;

        // tarea
        $this->tarea = new DbField(
            $this, // Table
            'x_tarea', // Variable name
            'tarea', // Name
            '`tarea`', // Expression
            '`tarea`', // Basic search expression
            200, // Type
            65535, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`tarea`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->tarea->InputTextType = "text";
        $this->tarea->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['tarea'] = &$this->tarea;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "horas_trabajadas";
    }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->socio_id->DbValue = $row['socio_id'];
        $this->fecha->DbValue = $row['fecha'];
        $this->horas->DbValue = $row['horas'];
        $this->tarea->DbValue = $row['tarea'];
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("HorasTrabajadasList");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "HorasTrabajadasView" => $this->language->phrase("View"),
            "HorasTrabajadasEdit" => $this->language->phrase("Edit"),
            "HorasTrabajadasAdd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "HorasTrabajadasView",
            Config("API_ADD_ACTION") => "HorasTrabajadasAdd",
            Config("API_EDIT_ACTION") => "HorasTrabajadasEdit",
            Config("API_DELETE_ACTION") => "HorasTrabajadasDelete",
            Config("API_LIST_ACTION") => "HorasTrabajadasList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("HorasTrabajadasView", $parm);
        } else {
            $url = $this->keyUrl("HorasTrabajadasView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "HorasTrabajadasAdd?" . $parm;
        } else {
            $url = "HorasTrabajadasAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("HorasTrabajadasEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("HorasTrabajadasList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("HorasTrabajadasAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("HorasTrabajadasList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("HorasTrabajadasDelete", $parm);
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->socio_id);
                    $doc->exportCaption($this->fecha);
                    $doc->exportCaption($this->horas);
                    $doc->exportCaption($this->tarea);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->socio_id);
                        $doc->exportField($this->fecha);
                        $doc->exportField($this->horas);
                        $doc->exportField($this->tarea);
