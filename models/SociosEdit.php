 ...
 * Page class
 */
class SociosEdit extends Socios
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "SociosEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "SociosEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("SociosList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "SociosList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "SociosList") {
                            ...
                            $returnUrl = "SociosList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'fecha_ingreso' before field var 'x_fecha_ingreso'
        $val = $this->getFormValue("fecha_ingreso", null) ?? $this->getFormValue("x_fecha_ingreso", null);
        if (!$this->fecha_ingreso->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->fecha_ingreso->Visible = false; // Disable update for API request
            } else {
                $this->fecha_ingreso->setFormValue($val, true, $validate);
            }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // fecha_ingreso
            $this->fecha_ingreso->setupEditAttributes();
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->fecha_ingreso->Visible && $this->fecha_ingreso->Required) {
                if (!$this->fecha_ingreso->IsDetailKey && IsEmpty($this->fecha_ingreso->FormValue)) {
                    $this->fecha_ingreso->addErrorMessage(str_replace("%s", $this->fecha_ingreso->caption(), $this->fecha_ingreso->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // cedula
        if ($this->security->canAdmin()) { // System admin
            $this->cedula->setDbValueDef($newRow, $this->cedula->CurrentValue, $this->cedula->ReadOnly);
        }
        ...
        if (!IsMaskedPassword($this->contrasena->CurrentValue)) {
            $this->contrasena->setDbValueDef($newRow, $this->contrasena->CurrentValue, $this->contrasena->ReadOnly || Config("ENCRYPTED_PASSWORD") && $oldRow['contraseña'] == $this->contrasena->CurrentValue);
        }
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['contraseña'])) { // contraseña
            $this->contrasena->CurrentValue = $row['contraseña'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("SociosList"), "", $this->TableVar, true);
        $pageId = "edit";
