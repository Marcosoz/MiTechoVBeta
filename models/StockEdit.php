...
class StockEdit extends Stock
{
    ...
    // Page object name
    public string $PageObjName = "StockEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "StockEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("StockList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "StockList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "StockList") {
                            ...
                            $returnUrl = "StockList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'nombre_material' before field var 'x_nombre_material'
        $val = $this->getFormValue("nombre_material", null) ?? $this->getFormValue("x_nombre_material", null);
        if (!$this->nombre_material->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->nombre_material->Visible = false; // Disable update for API request
            } else {
                $this->nombre_material->setFormValue($val);
            }
        }

        // Check field name 'unidad' before field var 'x_unidad'
        $val = $this->getFormValue("unidad", null) ?? $this->getFormValue("x_unidad", null);
        if (!$this->unidad->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->unidad->Visible = false; // Disable update for API request
            } else {
                $this->unidad->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->nombre_material->CurrentValue = $this->nombre_material->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        // nombre_material
        $this->nombre_material->RowCssClass = "row";

        // unidad
        $this->unidad->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // nombre_material
            $this->nombre_material->setupEditAttributes();
            $this->nombre_material->EditValue = !$this->nombre_material->Raw ? HtmlDecode($this->nombre_material->CurrentValue) : $this->nombre_material->CurrentValue;
            $this->nombre_material->PlaceHolder = RemoveHtml($this->nombre_material->caption());

            // unidad
            $this->unidad->setupEditAttributes();
            $this->unidad->EditValue = !$this->unidad->Raw ? HtmlDecode($this->unidad->CurrentValue) : $this->unidad->CurrentValue;
            $this->unidad->PlaceHolder = RemoveHtml($this->unidad->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->nombre_material->Visible && $this->nombre_material->Required) {
                if (!$this->nombre_material->IsDetailKey && IsEmpty($this->nombre_material->FormValue)) {
                    $this->nombre_material->addErrorMessage(str_replace("%s", $this->nombre_material->caption(), $this->nombre_material->RequiredErrorMessage));
                }
            }
            if ($this->unidad->Visible && $this->unidad->Required) {
                if (!$this->unidad->IsDetailKey && IsEmpty($this->unidad->FormValue)) {
                    $this->unidad->addErrorMessage(str_replace("%s", $this->unidad->caption(), $this->unidad->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // nombre_material
        $this->nombre_material->setDbValueDef($newRow, $this->nombre_material->CurrentValue, $this->nombre_material->ReadOnly);

        // unidad
        $this->unidad->setDbValueDef($newRow, $this->unidad->CurrentValue, $this->unidad->ReadOnly);

        // cantidad
        $this->cantidad->setDbValueDef($newRow, $this->cantidad->CurrentValue, $this->cantidad->ReadOnly);
    ...
    {
        if (isset($row['nombre_material'])) { // nombre_material
            $this->nombre_material->CurrentValue = $row['nombre_material'];
        }
        if (isset($row['unidad'])) { // unidad
            $this->unidad->CurrentValue = $row['unidad'];
        }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("StockList"), "", $this->TableVar, true);
        $pageId = "edit";
