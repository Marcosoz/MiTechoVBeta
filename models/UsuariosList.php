 ...
 * Page class
 */
class UsuariosList extends Usuarios
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "UsuariosList";
    ...
    public string $FormName = "fusuarioslist";

    // CSS class/style
    public string $CurrentPageName = "UsuariosList";
    ...
    public function setVisibility(): void
    {
        ...
        $this->nombre_completo->setVisibility();
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        if (!isset($GLOBALS["usuarios"]) || $GLOBALS["usuarios"]::class == PROJECT_NAMESPACE . "usuarios") {
        ...
        // Initialize URLs
        $this->AddUrl = "UsuariosAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "UsuariosDelete";
        $this->MultiUpdateUrl = "UsuariosUpdate";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "UsuariosView"); // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList(): string
    {
        ...
        $filterList = Concat($filterList, $this->nombre_completo->AdvancedSearch->toJson(), ","); // Field nombre_completo
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fusuariossrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        $this->nombre_completo->AdvancedSearch->SearchValue = $filter["x_nombre_completo"] ?? "";
        $this->nombre_completo->AdvancedSearch->SearchOperator = $filter["z_nombre_completo"] ?? "";
        $this->nombre_completo->AdvancedSearch->SearchCondition = $filter["v_nombre_completo"] ?? "";
        $this->nombre_completo->AdvancedSearch->SearchValue2 = $filter["y_nombre_completo"] ?? "";
        $this->nombre_completo->AdvancedSearch->SearchOperator2 = $filter["w_nombre_completo"] ?? "";
        $this->nombre_completo->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->username;
        $searchFlds[] = &$this->password;
        $searchFlds[] = &$this->nombre_completo;
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fusuarioslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fusuarioslist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"usuarios\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "nombre_completo");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fusuariossrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fusuariossrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fusuarioslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_usuarios", "data-rowtype" => RowType::ADD]);
    ...
    protected function newRow(): array
    {
        ...
        $row['username'] = $this->username->DefaultValue;
        $row['password'] = $this->password->DefaultValue;
    ...
    protected function setupSearchOptions(): void
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fusuariossrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $this->language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
