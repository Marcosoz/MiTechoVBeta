 ...
 * Page class
 */
class HorasTrabajadasList extends HorasTrabajadas
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "HorasTrabajadasList";
    ...
    public string $FormName = "fhoras_trabajadaslist";

    // CSS class/style
    public string $CurrentPageName = "HorasTrabajadasList";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->TableVar = 'horas_trabajadas';
        $this->TableName = 'horas_trabajadas';
        ...
        // Table object (horas_trabajadas)
        if (!isset($GLOBALS["horas_trabajadas"]) || $GLOBALS["horas_trabajadas"]::class == PROJECT_NAMESPACE . "horas_trabajadas") {
            $GLOBALS["horas_trabajadas"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "HorasTrabajadasAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "HorasTrabajadasDelete";
        $this->MultiUpdateUrl = "HorasTrabajadasUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'horas_trabajadas');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "HorasTrabajadasView"); // If View page, no primary button
                } else { // List page
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fhoras_trabajadassrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field tarea
        $this->tarea->AdvancedSearch->SearchValue = $filter["x_tarea"] ?? "";
        $this->tarea->AdvancedSearch->SearchOperator = $filter["z_tarea"] ?? "";
        $this->tarea->AdvancedSearch->SearchCondition = $filter["v_tarea"] ?? "";
        $this->tarea->AdvancedSearch->SearchValue2 = $filter["y_tarea"] ?? "";
        $this->tarea->AdvancedSearch->SearchOperator2 = $filter["w_tarea"] ?? "";
        $this->tarea->AdvancedSearch->save();
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fhoras_trabajadaslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fhoras_trabajadaslist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"horas_trabajadas\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fhoras_trabajadassrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fhoras_trabajadassrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fhoras_trabajadaslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_horas_trabajadas", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_horas_trabajadas",
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->horas->ViewValue = FormatNumber($this->horas->ViewValue, $this->horas->formatPattern());

            // tarea
            $this->tarea->ViewValue = $this->tarea->CurrentValue;
            ...
            // tarea
            $this->tarea->HrefValue = "";
            $this->tarea->TooltipValue = "";
