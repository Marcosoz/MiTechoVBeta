 ...
 * Page class
 */
class HorasTrabajadasEdit extends HorasTrabajadas
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "HorasTrabajadasEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "HorasTrabajadasEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("HorasTrabajadasList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "HorasTrabajadasList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "HorasTrabajadasList") {
                            ...
                            $returnUrl = "HorasTrabajadasList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues(): void
    {
        ...
        if (!$this->socio_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->socio_id->Visible = false; // Disable update for API request
            } else {
                $this->socio_id->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'horas' before field var 'x_horas'
        $val = $this->getFormValue("horas", null) ?? $this->getFormValue("x_horas", null);
        ...
        // Check field name 'tarea' before field var 'x_tarea'
        $val = $this->getFormValue("tarea", null) ?? $this->getFormValue("x_tarea", null);
        if (!$this->tarea->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tarea->Visible = false; // Disable update for API request
            } else {
                $this->tarea->setFormValue($val);
            }
        }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->socio_id->PlaceHolder = RemoveHtml($this->socio_id->caption());
            if (strval($this->socio_id->EditValue) != "" && is_numeric($this->socio_id->EditValue)) {
                $this->socio_id->EditValue = FormatNumber($this->socio_id->EditValue, null);
            }
            ...
            if (strval($this->horas->EditValue) != "" && is_numeric($this->horas->EditValue)) {
                $this->horas->EditValue = FormatNumber($this->horas->EditValue, null);
            }

            // tarea
            $this->tarea->setupEditAttributes();
            $this->tarea->EditValue = !$this->tarea->Raw ? HtmlDecode($this->tarea->CurrentValue) : $this->tarea->CurrentValue;
            $this->tarea->PlaceHolder = RemoveHtml($this->tarea->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if (!CheckInteger($this->socio_id->FormValue)) {
                $this->socio_id->addErrorMessage($this->socio_id->getErrorMessage(false));
            }
            ...
            if ($this->horas->Visible && $this->horas->Required) {
                if (!$this->horas->IsDetailKey && IsEmpty($this->horas->FormValue)) {
            ...
            if ($this->tarea->Visible && $this->tarea->Required) {
                if (!$this->tarea->IsDetailKey && IsEmpty($this->tarea->FormValue)) {
                    $this->tarea->addErrorMessage(str_replace("%s", $this->tarea->caption(), $this->tarea->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // horas
        $this->horas->setDbValueDef($newRow, $this->horas->CurrentValue, $this->horas->ReadOnly);

        // tarea
        $this->tarea->setDbValueDef($newRow, $this->tarea->CurrentValue, $this->tarea->ReadOnly);
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['tarea'])) { // tarea
            $this->tarea->CurrentValue = $row['tarea'];
        }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("HorasTrabajadasList"), "", $this->TableVar, true);
        $pageId = "edit";
