 ...
 * Page class
 */
class ComprasAdd extends Compras
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "ComprasAdd";
    ...
    // CSS class/style
    public string $CurrentPageName = "ComprasAdd";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        if (!isset($GLOBALS["compras"]) || $GLOBALS["compras"]::class == PROJECT_NAMESPACE . "compras") {
            $GLOBALS["compras"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'compras');
        }
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (
                    ...
                    if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                        ...
                        $result["view"] = SameString($pageName, "ComprasView"); // If View page, no primary button
                    } else { // List page
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$oldRow) { // Record not loaded
                    ...
                    $this->terminate("ComprasList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                if ($this->addRow($oldRow)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ComprasList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ComprasView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ComprasList") {
                            ...
                            $returnUrl = "ComprasList"; // Return list page content
                        }
                    }
    ...
    {
        $this->saldo_pendiente->DefaultValue = $this->saldo_pendiente->getDefault(); // PHP
        $this->saldo_pendiente->OldValue = $this->saldo_pendiente->DefaultValue;
    }
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'proveedor_id' before field var 'x_proveedor_id'
        $val = $this->getFormValue("proveedor_id", null) ?? $this->getFormValue("x_proveedor_id", null);
        if (!$this->proveedor_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->proveedor_id->Visible = false; // Disable update for API request
            } else {
                $this->proveedor_id->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'descripcion' before field var 'x_descripcion'
        $val = $this->getFormValue("descripcion", null) ?? $this->getFormValue("x_descripcion", null);
        ...
        // Check field name 'monto' before field var 'x_monto'
        $val = $this->getFormValue("monto", null) ?? $this->getFormValue("x_monto", null);
        ...
        // Check field name 'saldo_pendiente' before field var 'x_saldo_pendiente'
        $val = $this->getFormValue("saldo_pendiente", null) ?? $this->getFormValue("x_saldo_pendiente", null);
        if (!$this->saldo_pendiente->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->saldo_pendiente->Visible = false; // Disable update for API request
            } else {
                $this->saldo_pendiente->setFormValue($val, true, $validate);
            }
        }
    ...
    {
        $this->proveedor_id->CurrentValue = $this->proveedor_id->FormValue;
        $this->fecha->CurrentValue = $this->fecha->FormValue;
        $this->fecha->CurrentValue = UnformatDateTime($this->fecha->CurrentValue, $this->fecha->formatPattern());
        $this->descripcion->CurrentValue = $this->descripcion->FormValue;
        $this->monto->CurrentValue = $this->monto->FormValue;
        $this->saldo_pendiente->CurrentValue = $this->saldo_pendiente->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        // proveedor_id
        $this->proveedor_id->RowCssClass = "row";
        ...
        // saldo_pendiente
        $this->saldo_pendiente->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->proveedor_id->ViewValue = FormatNumber($this->proveedor_id->ViewValue, $this->proveedor_id->formatPattern());
            ...
            // saldo_pendiente
            $this->saldo_pendiente->ViewValue = $this->saldo_pendiente->CurrentValue;
            $this->saldo_pendiente->ViewValue = FormatNumber($this->saldo_pendiente->ViewValue, $this->saldo_pendiente->formatPattern());
            ...
            // saldo_pendiente
            $this->saldo_pendiente->HrefValue = "";

            // created_at
            $this->created_at->HrefValue = "";
        } elseif ($this->RowType == RowType::ADD) {
            // proveedor_id
            $this->proveedor_id->setupEditAttributes();
            $this->proveedor_id->EditValue = $this->proveedor_id->CurrentValue;
            $this->proveedor_id->PlaceHolder = RemoveHtml($this->proveedor_id->caption());
            if (strval($this->proveedor_id->EditValue) != "" && is_numeric($this->proveedor_id->EditValue)) {
                $this->proveedor_id->EditValue = FormatNumber($this->proveedor_id->EditValue, null);
            }
            ...
            // saldo_pendiente
            $this->saldo_pendiente->setupEditAttributes();
            $this->saldo_pendiente->EditValue = $this->saldo_pendiente->CurrentValue;
            $this->saldo_pendiente->PlaceHolder = RemoveHtml($this->saldo_pendiente->caption());
            if (strval($this->saldo_pendiente->EditValue) != "" && is_numeric($this->saldo_pendiente->EditValue)) {
                $this->saldo_pendiente->EditValue = FormatNumber($this->saldo_pendiente->EditValue, null);
            }
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->proveedor_id->Visible && $this->proveedor_id->Required) {
                if (!$this->proveedor_id->IsDetailKey && IsEmpty($this->proveedor_id->FormValue)) {
                    $this->proveedor_id->addErrorMessage(str_replace("%s", $this->proveedor_id->caption(), $this->proveedor_id->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->proveedor_id->FormValue)) {
                $this->proveedor_id->addErrorMessage($this->proveedor_id->getErrorMessage(false));
            }
            ...
            if ($this->monto->Visible && $this->monto->Required) {
                if (!$this->monto->IsDetailKey && IsEmpty($this->monto->FormValue)) {
            ...
            if ($this->saldo_pendiente->Visible && $this->saldo_pendiente->Required) {
                if (!$this->saldo_pendiente->IsDetailKey && IsEmpty($this->saldo_pendiente->FormValue)) {
                    $this->saldo_pendiente->addErrorMessage(str_replace("%s", $this->saldo_pendiente->caption(), $this->saldo_pendiente->RequiredErrorMessage));
                }
            }
            if (!CheckNumber($this->saldo_pendiente->FormValue)) {
                $this->saldo_pendiente->addErrorMessage($this->saldo_pendiente->getErrorMessage(false));
            }
    ...
    protected function getAddRow(): array
    {
        ...
        // proveedor_id
        $this->proveedor_id->setDbValueDef($newRow, $this->proveedor_id->CurrentValue, false);

        // fecha
        $this->fecha->setDbValueDef($newRow, UnFormatDateTime($this->fecha->CurrentValue, $this->fecha->formatPattern()), false);

        // descripcion
        $this->descripcion->setDbValueDef($newRow, $this->descripcion->CurrentValue, false);

        // monto
        $this->monto->setDbValueDef($newRow, $this->monto->CurrentValue, false);

        // saldo_pendiente
        $this->saldo_pendiente->setDbValueDef($newRow, $this->saldo_pendiente->CurrentValue, strval($this->saldo_pendiente->CurrentValue) == "");
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ComprasList"), "", $this->TableVar, true);
