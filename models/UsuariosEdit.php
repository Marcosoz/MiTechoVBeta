 ...
 * Page class
 */
class UsuariosEdit extends Usuarios
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "UsuariosEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "UsuariosEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("UsuariosList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "UsuariosList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "UsuariosList") {
                            ...
                            $returnUrl = "UsuariosList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'username' before field var 'x_username'
        $val = $this->getFormValue("username", null) ?? $this->getFormValue("x_username", null);
        ...
        $val = $this->getFormValue("password", null) ?? $this->getFormValue("x_password", null);
        ...
        $val = $this->getFormValue("userlevel", null) ?? $this->getFormValue("x__userlevel", null);
        if (!$this->_userlevel->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->_userlevel->Visible = false; // Disable update for API request
            } else {
                $this->_userlevel->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // username
            $this->username->setupEditAttributes();
            $this->username->EditValue = !$this->username->Raw ? HtmlDecode($this->username->CurrentValue) : $this->username->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->username->Visible && $this->username->Required) {
                if (!$this->username->IsDetailKey && IsEmpty($this->username->FormValue)) {
                    $this->username->addErrorMessage(str_replace("%s", $this->username->caption(), $this->username->RequiredErrorMessage));
                }
            }
            if ($this->password->Visible && $this->password->Required) {
    ...
    protected function editRow(): bool
    {
        ...
        if ($this->username->CurrentValue != "") {
            $filterChk = "(`username` = '" . AdjustSql($this->username->CurrentValue) . "')";
            ...
            if ($rsChk->fetchAssociative()) {
                $idxErrMsg = sprintf($this->language->phrase("DuplicateIndex"), $this->username->CurrentValue, $this->username->caption());
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // username
        $this->username->setDbValueDef($newRow, $this->username->CurrentValue, $this->username->ReadOnly);

        // password
        $this->password->setDbValueDef($newRow, $this->password->CurrentValue, $this->password->ReadOnly);

        // nombre_completo
        $this->nombre_completo->setDbValueDef($newRow, $this->nombre_completo->CurrentValue, $this->nombre_completo->ReadOnly);
        ...
        // userlevel
        $this->_userlevel->setDbValueDef($newRow, $this->_userlevel->CurrentValue, $this->_userlevel->ReadOnly);
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        if (isset($row['username'])) { // username
            $this->username->CurrentValue = $row['username'];
        }
        if (isset($row['password'])) { // password
            $this->password->CurrentValue = $row['password'];
        }
        if (isset($row['nombre_completo'])) { // nombre_completo
            $this->nombre_completo->CurrentValue = $row['nombre_completo'];
        }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("UsuariosList"), "", $this->TableVar, true);
        $pageId = "edit";
