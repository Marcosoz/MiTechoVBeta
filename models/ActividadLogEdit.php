 ...
 * Page class
 */
class ActividadLogEdit extends ActividadLog
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "ActividadLogEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "ActividadLogEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ActividadLogList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "ActividadLogList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ActividadLogList") {
                            ...
                            $returnUrl = "ActividadLogList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'usuario_id' before field var 'x_usuario_id'
        $val = $this->getFormValue("usuario_id", null) ?? $this->getFormValue("x_usuario_id", null);
        if (!$this->usuario_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->usuario_id->Visible = false; // Disable update for API request
            } else {
                $this->usuario_id->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'accion' before field var 'x_accion'
        $val = $this->getFormValue("accion", null) ?? $this->getFormValue("x_accion", null);
        ...
        // Check field name 'detalles' before field var 'x_detalles'
        $val = $this->getFormValue("detalles", null) ?? $this->getFormValue("x_detalles", null);
        if (!$this->detalles->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->detalles->Visible = false; // Disable update for API request
            } else {
                $this->detalles->setFormValue($val);
            }
        }
    ...
    public function renderRow(): void
    {
        ...
        // usuario_id
        $this->usuario_id->RowCssClass = "row";

        // cooperativa_id
        $this->cooperativa_id->RowCssClass = "row";

        // accion
        $this->accion->RowCssClass = "row";

        // detalles
        $this->detalles->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (strval($this->usuario_id->EditValue) != "" && is_numeric($this->usuario_id->EditValue)) {
                $this->usuario_id->EditValue = FormatNumber($this->usuario_id->EditValue, null);
            }
            ...
            // accion
            $this->accion->setupEditAttributes();
            $this->accion->EditValue = !$this->accion->Raw ? HtmlDecode($this->accion->CurrentValue) : $this->accion->CurrentValue;
            $this->accion->PlaceHolder = RemoveHtml($this->accion->caption());

            // detalles
            $this->detalles->setupEditAttributes();
            $this->detalles->EditValue = !$this->detalles->Raw ? HtmlDecode($this->detalles->CurrentValue) : $this->detalles->CurrentValue;
            $this->detalles->PlaceHolder = RemoveHtml($this->detalles->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->usuario_id->Visible && $this->usuario_id->Required) {
                if (!$this->usuario_id->IsDetailKey && IsEmpty($this->usuario_id->FormValue)) {
                    $this->usuario_id->addErrorMessage(str_replace("%s", $this->usuario_id->caption(), $this->usuario_id->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->usuario_id->FormValue)) {
                $this->usuario_id->addErrorMessage($this->usuario_id->getErrorMessage(false));
            }
            ...
            if ($this->accion->Visible && $this->accion->Required) {
                if (!$this->accion->IsDetailKey && IsEmpty($this->accion->FormValue)) {
                    $this->accion->addErrorMessage(str_replace("%s", $this->accion->caption(), $this->accion->RequiredErrorMessage));
                }
            }
            if ($this->detalles->Visible && $this->detalles->Required) {
                if (!$this->detalles->IsDetailKey && IsEmpty($this->detalles->FormValue)) {
                    $this->detalles->addErrorMessage(str_replace("%s", $this->detalles->caption(), $this->detalles->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // usuario_id
        $this->usuario_id->setDbValueDef($newRow, $this->usuario_id->CurrentValue, $this->usuario_id->ReadOnly);

        // cooperativa_id
        $this->cooperativa_id->setDbValueDef($newRow, $this->cooperativa_id->CurrentValue, $this->cooperativa_id->ReadOnly);

        // accion
        $this->accion->setDbValueDef($newRow, $this->accion->CurrentValue, $this->accion->ReadOnly);

        // detalles
        $this->detalles->setDbValueDef($newRow, $this->detalles->CurrentValue, $this->detalles->ReadOnly);
    ...
    {
        if (isset($row['usuario_id'])) { // usuario_id
            $this->usuario_id->CurrentValue = $row['usuario_id'];
        }
        if (isset($row['cooperativa_id'])) { // cooperativa_id
            $this->cooperativa_id->CurrentValue = $row['cooperativa_id'];
        }
        if (isset($row['accion'])) { // accion
            $this->accion->CurrentValue = $row['accion'];
        }
        if (isset($row['detalles'])) { // detalles
            $this->detalles->CurrentValue = $row['detalles'];
        }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ActividadLogList"), "", $this->TableVar, true);
        $pageId = "edit";
