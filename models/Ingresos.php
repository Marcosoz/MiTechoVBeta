...
/**
 * Table class for ingresos
 */
class Ingresos extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "ingresos"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // tipo_ingreso
        $this->tipo_ingreso = new DbField(
            $this, // Table
            'x_tipo_ingreso', // Variable name
            'tipo_ingreso', // Name
            '`tipo_ingreso`', // Expression
            '`tipo_ingreso`', // Basic search expression
            200, // Type
            9, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`tipo_ingreso`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->tipo_ingreso->InputTextType = "text";
        $this->tipo_ingreso->Raw = true;
        $this->tipo_ingreso->Nullable = false; // NOT NULL field
        $this->tipo_ingreso->Required = true; // Required field
        $this->tipo_ingreso->Lookup = new Lookup($this->tipo_ingreso, 'ingresos', false, '', ["","","",""], '', "", [], [], [], [], [], [], false, '', '', "");
        $this->tipo_ingreso->OptionCount = 5;
        $this->tipo_ingreso->SearchOperators = ["=", "<>"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "ingresos";
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("IngresosList");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "IngresosView" => $this->language->phrase("View"),
            "IngresosEdit" => $this->language->phrase("Edit"),
            "IngresosAdd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "IngresosView",
            Config("API_ADD_ACTION") => "IngresosAdd",
            Config("API_EDIT_ACTION") => "IngresosEdit",
            Config("API_DELETE_ACTION") => "IngresosDelete",
            Config("API_LIST_ACTION") => "IngresosList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("IngresosView", $parm);
        } else {
            $url = $this->keyUrl("IngresosView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("IngresosEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("IngresosList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("IngresosAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("IngresosList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("IngresosDelete", $parm);
        }
    }
